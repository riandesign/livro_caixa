...
if (isset($_POST['acao']) && $_POST['acao'] == 'editar_cat') {
    ...
    mysqli_query("UPDATE lc_cat SET nome='$nome' WHERE id='$id'");
    echo mysqli_error();
...
if (isset($_GET['acao']) && $_GET['acao'] == 'apagar_cat') {
    ...
    $qr=mysqli_query("SELECT c.id FROM lc_movimento m, lc_cat c WHERE c.id=m.cat && c.id=$id");
    if (mysqli_num_rows($qr)>0){
        header("Location: ?mes=" . $_GET['mes'] . "&ano=" . $_GET['ano'] . "&cat_err=1");
        exit();
    }

    mysqli_query("DELETE FROM lc_cat WHERE id='$id'");
    echo mysqli_error();
...
<?php
$qr=mysqli_query("SELECT id, nome FROM lc_cat");
...
<?php
$qr=mysqli_query("SELECT * FROM lc_cat");
if (mysqli_num_rows($qr)==0)
...
<?php
$qr=mysqli_query("SELECT SUM(valor) as total FROM lc_movimento WHERE tipo=1 && mes='$mes_hoje' && ano='$ano_hoje'");
$row=mysqli_fetch_array($qr);
$entradas=$row['total'];

$qr=mysqli_query("SELECT SUM(valor) as total FROM lc_movimento WHERE tipo=0 && mes='$mes_hoje' && ano='$ano_hoje'");
$row=mysqli_fetch_array($qr);
$saidas=$row['total'];
